...
write; `TestReadOnly` включает test без write. Alpha-test пока завершается
явной ошибкой: shader variant ещё не существует, поэтому renderer не должен
молча подменять state. Также не установлен источник значений для Batch20/MAT0: их
